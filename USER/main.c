...
int main(void)
{
	...
	memset(&humiture_data, 0, sizeof(humitureDataTypeDef));//申请内存空间
	...
	{
		/* 温湿度报警设置处理 */
		humiture_alarm_set_dispose();//界面设计（切换）
		...
		if (humiture_data.show_flag == 0)
		{
			...
			//返回0读取正常，1读取失败
			if (DHT11_Read_Data(&temp, &humidity) == 0)//读取到了温湿度值
			{
			...
			{//水量充足且蜂鸣器没有报警
				...
				else
				{
					...
					humiture_data.alarm_flag = 1;//开启报警
        ...
        {
			__set_PRIMASK(1);
			 //PRIMASK 只有单一比特的寄存器。置为1后,就关掉所有可屏蔽异常,只剩下NMI和硬Fault可以响应
			//蓝牙无线通讯，波特率为9600
...
{//通道2	
	...
	for(t=0;t<times;t++)
	{
		temp_val+=Get_Adc(ch);//返回转换的湿度值//这个是获取在ADC寄存器保存的通道值
		delay_ms(5);
	}
...
{
	return Get_Adc_Average(2, 2);//返回AD转换的值0~4095
}

//温湿度报警设置处理
void humiture_alarm_set_dispose(void)
{
	u8 key = KEY_NULL;//开始无按键按下
	char str1[8] = {0};
	char humiture_str[16] = {0};
	//两个页面，主界面page和湿度界面page
	if (humiture_data.page == 0)
	{
		...
		{//长按，短按操作。
			case KEY1_LONG_PRESS:
				...
				humidity_interface();//进入界面2
				
				break;
			//加湿器模式开/关切换
			case KEY3_SHORT_PRESS:
				
				if (mode_flag == 1)//mode_flag模式标志位：自动模式为1，手动模式为0
				{
